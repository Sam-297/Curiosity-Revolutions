...
void generation_aleatoire(Terrain *T, int l, int h, float dObst) {
  int x,y,eau_or_rocher;
  T->largeur=l;
  T->hauteur=h;
  int nb_obs= (int) (((float) (h) * (float) (l) * dObst)+ 0.5);
  for (int i = 0 ; i < l; i++){
    for (int j = 0; j < h; j++){
      T->tab[i][j]=LIBRE;
    }
  }
  for (int compteur = 0; compteur < nb_obs; compteur++){
    x=rand()%l;
    y=rand()%h;
    ...
    if (x==l/2&&y==h/2){
    ...
    else if (T->tab[x][y]==EAU || T->tab[x][y]==ROCHER){
      compteur--;
    }
    else{
      eau_or_rocher=rand()%2;
      switch (eau_or_rocher){
      case 0:
        T->tab[x][y]=EAU;
        break;
      case 1:
      ...
      if (!existe_chemin_vers_sortie(T)){
        // compteur--; // ! boucle infinie si densite grande
        T->tab[x][y]=LIBRE;
      }
    }
  }
